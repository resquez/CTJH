...
document.querySelector(".submit-btn")?.addEventListener("click", function () {
    ...
    if (selectedCheckbox) {
        ...
        // ✅ API 요청 URL 생성
        const apiUrl = `http://192.168.1.101:5000/api/main/list/detail/${flightId}?passenger_count=${passengerCount}`;
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                // ✅ 응답 데이터 확인 후 페이지 이동
                const redirectUrl = `http://192.168.1.100:80/main/main_list_detail.html?flight_id=${flightId}&passenger_count=${passengerCount}`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ✅ API 요청 URL 생성
    const apiUrl = `http://192.168.1.101:5000/api/main/list?departure_airport=${encodeURIComponent(departure_airport)}&arrival_airport=${encodeURIComponent(arrival_airport)}&departure_date=${encodeURIComponent(departure_date)}&seat_class=${encodeURIComponent(seat_class)}&passenger_count=${encodeURIComponent(passenger_count)}`;
