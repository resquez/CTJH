...
//    const isAdmin = true;  // ✅ JavaScript에서는 `true`로 써야 함
   ...
   // const qnaId = urlParams.get('id');
    ...
    fetch("http://192.168.1.101:5000/api/member/status", {  // ⭐️ 관리자 여부 체크
            method: "GET",
            credentials: "include"
        })
        .then(response => response.json())
    ...
    if (answerBtn) {
        answerBtn.addEventListener("click", function () {
            ...
            fetch(`http://192.168.1.101:5000/api/qna/comment/${qnaId}`, {
                method: "POST",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener("click", function () {
            ...
            fetch(`http://192.168.1.101:5000/api/qna/delete/${qnaId}`, {
                method: "DELETE"
            })
            .then(response => response.json())
            .then(data => {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (backBtn) {
        backBtn.addEventListener("click", function () {
            window.location.href = "http://192.168.1.100:80/qna/qna.html";  // ✅ 목록 페이지 URL로 변경
        });
    }
});
